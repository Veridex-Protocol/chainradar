"""Collector for Hyperlane Registry."""
...
class HyperlaneRegistryCollector:
    source_id: str = "hyperlane_registry"
    family: str = "registry"
    ...
    async def fetch(self, cursor: Cursor, budget: RateBudget) -> FetchBatch:
        ...
        url = "https://api.github.com/repos/hyperlane-xyz/hyperlane-registry/pulls?state=all&sort=updated&direction=desc&per_page=15"
        items: List[RawItem] = []
            ...
            if resp.status_code == 200:
                ...
                for pr in prs:
                    ...
                    items.append(
                        RawItem(
                            source_id=self.source_id,
                            external_id=f"hyperlane_pr_{pr_num}_{updated_at_str}",
                            url=pr.get("html_url") or f"https://github.com/hyperlane-xyz/hyperlane-registry/pull/{pr_num}",
                            ...
                            content_hash=c_hash,
                            provenance={"source": "hyperlane-registry", "pr_number": pr_num},
                        )
                    )
        except Exception:
            pass
    ...
    def normalize(self, raw: RawItem) -> List[RawObservation]:
        ...
        name = title.replace("Add", "").replace("add", "").replace("chain", "").replace("metadata", "").strip()
        if not name or len(name) < 2:
            name = f"Hyperlane PR #{payload.get('pr_number', 'unknown')}"

        import re
        m_id = re.search(r"chainId[:\s=]+(\d+)", combined, re.IGNORECASE)
        ...
        if "sealevel" in combined.lower() or "solana" in combined.lower():
            stack_family = StackFamily.SVM
        elif "cosmos" in combined.lower():
        ...
        obs = RawObservation(
            ...
            environment=env,
            is_public=True,
            caip2=format_caip2("eip155", chain_id) if (chain_id and stack_family == StackFamily.EVM) else None,
            protocol_namespace="eip155" if stack_family == StackFamily.EVM else stack_family.value,
            human_chain_id=chain_id,
            rpc_urls=list(set(rpcs)),
            explorer_urls=list(set(explorers)),
            stage=stage,
            raw_text=combined,
            claims={"pr_number": payload.get("pr_number"), "interoperability": "hyperlane"},
            reliability=0.88,
        )
        return [obs]
