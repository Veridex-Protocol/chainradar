"""Collector for Axelar Configs Registry."""
...
class AxelarConfigsCollector:
    source_id: str = "axelar_configs"
    family: str = "registry"
    ...
    async def fetch(self, cursor: Cursor, budget: RateBudget) -> FetchBatch:
        ...
        url = "https://api.github.com/repos/axelarnetwork/axelar-configs/pulls?state=all&sort=updated&direction=desc&per_page=15"
        items: List[RawItem] = []
            ...
            if resp.status_code == 200:
                ...
                for pr in prs:
                    ...
                    items.append(
                        RawItem(
                            source_id=self.source_id,
                            external_id=f"axelar_pr_{pr_num}_{updated_at_str}",
                            url=pr.get("html_url") or f"https://github.com/axelarnetwork/axelar-configs/pull/{pr_num}",
                            ...
                            content_hash=c_hash,
                            provenance={"source": "axelar-configs", "pr_number": pr_num},
                        )
                    )
        except Exception:
            pass
    ...
    def normalize(self, raw: RawItem) -> List[RawObservation]:
        ...
        if not name or len(name) < 2:
            name = f"Axelar PR #{payload.get('pr_number', 'unknown')}"
        ...
        obs = RawObservation(
            ...
            stage=stage,
            raw_text=combined,
            claims={"pr_number": payload.get("pr_number"), "interoperability": "axelar"},
            reliability=0.85,
        )
        return [obs]
