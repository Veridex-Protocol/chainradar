...
# PostgreSQL is the production store and needs JSONB so the spec's GIN indexes
# (spec 21) are possible; SQLite keeps plain JSON for fast local test runs.
...
class EvidenceEvent(Base):
    ...
    # Coarse time bucket completing the spec's three-column dedup key. Without
    # it, identical unchanged content could never be re-observed, which would
    # silently freeze freshness and liveness re-observation.
    observed_bucket: Mapped[str] = mapped_column(String(32), default="", nullable=False)
    raw_payload_path: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    extracted_claims: Mapped[Dict[str, Any]] = mapped_column(JSONType, default=dict, nullable=False)
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "source_id", "content_hash", "observed_bucket", name="uq_source_content_bucket"
        ),
        Index("ix_evidence_source_observed", "source_id", "observed_at"),
    )
...
class Organization(Base):
    ...
    official_domains: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    github_orgs: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    social_handles: Mapped[Dict[str, str]] = mapped_column(JSONType, default=dict, nullable=False)
    claimed_locations: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
...
class ChainProduct(Base):
    ...
    current_score_id: Mapped[Optional[str]] = mapped_column(
        String(64), ForeignKey("score_snapshots.id", use_alter=True, name="fk_chain_current_score"), nullable=True
    )
    ...
    # `score` is the current projection; `score_history` is the full ledger.
    score = relationship(
        "ScoreSnapshot",
        foreign_keys="ChainProduct.current_score_id",
        lazy="selectin",
        post_update=True,
    )
    score_history = relationship(
        "ScoreSnapshot",
        back_populates="chain_product",
        foreign_keys="ScoreSnapshot.candidate_id",
...
class Network(Base):
    ...
    rpc_urls: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    explorer_urls: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    faucet_urls: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    launch_claims: Mapped[Dict[str, Any]] = mapped_column(JSONType, default=dict, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False)
...
class Signal(Base):
    ...
    evidence_refs: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
...
class AfricaAssessment(Base):
    ...
    countries: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    regions: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    languages: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    rails_use_cases: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    evidence_ids: Mapped[List[str]] = mapped_column(JSONType, default=list, nullable=False)
    ...
    analyst_override: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSONType, nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False)
...
class ScoreSnapshot(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=generate_uuid)
    # NOT unique: every rescore appends a new snapshot so calibration,
    # backtesting and audit can replay how a decision changed (spec 20, 24).
    candidate_id: Mapped[str] = mapped_column(String(64), ForeignKey("chain_products.id"), index=True, nullable=False)
    confidence: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    ...
    feature_vector: Mapped[Dict[str, Any]] = mapped_column(JSONType, default=dict, nullable=False)
    ...
    chain_product = relationship("ChainProduct", back_populates="score_history", foreign_keys=[candidate_id], lazy="selectin")
...
class VerificationObservation(Base):
    """One read-only probe of an endpoint (spec 14).

    Liveness is proven by comparing two observations taken 60-180s apart, so
    *both* are persisted: an alert that claims a chain is advancing must be
    able to show the two heights it was derived from.
    """
    ...
    network_id: Mapped[Optional[str]] = mapped_column(
        String(64), ForeignKey("networks.id"), index=True, nullable=True
    )
    # Endpoint is stored with any embedded API key redacted (spec 15).
    endpoint: Mapped[str] = mapped_column(Text, nullable=False)
    endpoint_hash: Mapped[str] = mapped_column(String(64), index=True, nullable=False)
    family: Mapped[str] = mapped_column(String(32), nullable=False)
    probe_round: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    success: Mapped[bool] = mapped_column(Boolean, default=False, index=True, nullable=False)
    # One of the spec-15 failure classifications when success is false.
    failure_class: Mapped[Optional[str]] = mapped_column(String(32), index=True, nullable=True)
    failure_detail: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    identity_fingerprint: Mapped[Optional[str]] = mapped_column(String(128), index=True, nullable=True)
    chain_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    genesis_hash: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    block_height: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    client_version: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    raw_observation: Mapped[Dict[str, Any]] = mapped_column(JSONType, default=dict, nullable=False)
    observed_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, index=True, nullable=False
    )

    __table_args__ = (
        Index("ix_verification_candidate_observed", "candidate_id", "observed_at"),
    )


class ReviewTask(Base):
    """An analyst decision the engine refuses to make automatically.

    Spec 16 permits auto-merge only on strong technical or official
    relationships. Weak name/handle matches, chain-ID collisions and identity
    mismatches land here instead of being resolved silently.
    """
    ...
    related_candidate_id: Mapped[Optional[str]] = mapped_column(String(64), index=True, nullable=True)
    match_weight: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    ...
    __table_args__ = (
        # One open task per (type, candidate, counterparty) - re-observing the
        # same collision must not spawn a new task on every poll.
        UniqueConstraint(
            "task_type", "candidate_id", "related_candidate_id", "status", name="uq_open_review_task"
        ),
    )


class AlertDispatch(Base):
    """Record of an alert actually sent, backing the dedup window (spec 20)."""
    ...
    # Hash of the alert-worthy facts; an unchanged card inside the dedup
    # window is suppressed rather than re-sent.
    dedup_key: Mapped[str] = mapped_column(String(64), index=True, nullable=False)
    channel: Mapped[str] = mapped_column(String(64), default="webhook", nullable=False)
